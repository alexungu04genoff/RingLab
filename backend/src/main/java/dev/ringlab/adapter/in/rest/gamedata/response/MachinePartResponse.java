...
public record MachinePartResponse(
    ...
    String sourceMachineImagePath, RacingType racingType, MachineFamily sourceMachineFamily) {
  public static MachinePartResponse from(MachinePart part, Machine source) {
    return new MachinePartResponse(
        part.id(), part.type(), source.id(), source.name(), source.imagePath(), source.racingType(), source.family());
  }
}
