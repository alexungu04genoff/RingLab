...
The first start migrates the schema and seeds 53 racers, 27 source machines, 81 machine parts, and 20 gadgets. V6 expands the catalog using official release evidence through September 12, 2026, with local artwork for 52 racers and 22 machines. The racer roster is reconciled against official releases; the full named launch-machine inventory remains an evidence gap. See the [catalog source ledger](docs/game-data-sources.md) for every entry, missing artwork, and announced content excluded from production. Unverified racing types are null and display as Unknown. There are deliberately **no automatically seeded users or community builds**. Register through the UI, then create your first build. Registration signs you in immediately; login is also available separately. Use another browser/session to register a second account and try ownership restrictions.
...
Official racer and machine artwork lives in `frontend/public/assets/racers/` and `frontend/public/assets/machines/`. V6 assigns local paths; the [source ledger](docs/game-data-sources.md) records exact official assets and normalization. The UI displays an initials placeholder when a local image is absent or fails. New racing types remain unknown until verified; no game statistics are inferred from artwork.
