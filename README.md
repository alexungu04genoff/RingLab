...
PostgreSQL returns all filtered build candidates and raw vote counts. Domain `BuildRanking` and `WilsonScore` define ranking; `BuildService` ranks globally before pagination and derives the total from the candidate set. BEST_RATED uses full-precision Wilson confidence descending, then applies its Wilson-zero evidence rule, creation time descending and UUID ascending. This intentionally loads all matching candidates into memory. Comments remain paginated in PostgreSQL, with creation time ascending then UUID ascending.
