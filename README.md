...
The first start migrates the schema and seeds 53 racers, 27 source machines, 81 machine parts, and 71 gadgets. V6 expands racers/machines using official release evidence through September 12, 2026. V7 adds 51 verified gadget identities while retaining all 20 original IDs: 67 gadget identities are substantiated and four retained seed names require review. V9 uses user-approved Sonic Wiki artwork consistently for all 53 racers, 27 machines and 71 gadgets. Gadgets have 10 supported descriptions and nine latest verified costs; the remaining metadata stays null. The racer roster is reconciled against official releases; the complete named machine/gadget inventories remain evidence gaps. See the [catalog source ledger](docs/game-data-sources.md) for every entry, patch history, uncertainties, and announced content excluded from production. Unverified racing types are null and display as Unknown. There are deliberately **no automatically seeded users or community builds**. Register through the UI, then create your first build. Registration signs you in immediately; login is also available separately. Use another browser/session to register a second account and try ownership restrictions.
...
A build contains one racer, exactly one FRONT machine part, one REAR machine part, one TIRE machine part, and ordered gadgets. Each `MachinePart` originates from a source `Machine`; a machine supplies catalog metadata rather than being the build's single selected loadout. Parts may come from different source machines. Gadgets remain separate from machine parts, and their order is persisted with a position column; supported effects and latest verified costs are optional catalog metadata. There is no gadget capacity or compatibility validation and no calculated statistics. Historical costs live in the ledger, not in version-aware rules. The API preserves the submitted gadget list without guessing combination rules; the UI uses ordinary multi-selection and permits reordering. No additional game systems or community features are included.
