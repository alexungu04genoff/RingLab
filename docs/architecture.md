...
V4 creates three explicitly identified parts per seeded machine, backfills old builds, and removes `builds.machine_id`. V25 adds `machines.family`, marks all pre-existing rows `STANDARD`, and makes `tire_part_id` nullable for future Board builds. Build IDs, timestamps, ordered gadgets, votes, and comments are preserved.
...
The editor starts in Standard mode and offers an explicit Standard/Board family choice. Part options and stock shortcuts are filtered by family; switching family clears incompatible parts, and Board mode removes the tire selector. Details, cards, sharing, stats, remix/edit, and comparison handle a null tire and state that Boards do not use tires. The backend remains authoritative. The demo seeder continues to generate Standard builds because no verified Board catalog data is currently stored.
